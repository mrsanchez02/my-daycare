...
export function KidsDirectory({ sectionLabel, addKidLabel, searchPlaceholder, classroomLabel, kids }: KidsDirectoryProps) {
  ...
  const filteredKids = normalizedQuery
    ? kids.filter((kid) => kid.name.toLocaleLowerCase().includes(normalizedQuery))
    : kids;
  ...
  return (
    <section>
      ...
      {filteredKids.length > 0 ? (
        ...
        <div className="rounded-[18px] border border-dashed border-[#ddcfbc] bg-[#fff9f2] px-6 py-10 text-center text-[#8f8070]">
          <div className="font-[family:var(--font-geist-sans)] text-lg font-semibold text-[#3f362e]">No encontramos niños</div>
          <p className="mt-2 text-[14px]">Probá con otro nombre.</p>
        </div>
      )}
    </section>
  );
